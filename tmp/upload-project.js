...
async function uploadProject() {
  ...
  const tarGzPath = "/tmp/my-tech-blog-clean.tar.gz";

  if (!fs.existsSync(tarGzPath)) {
    console.error("❌ 文件不存在:", tarGzPath);
    return;
  }
  ...
  const fileKey = await storage.uploadFile({
    ...
    fileName: "my-tech-blog-clean.tar.gz",
  ...
  console.log("\n💡 下载后执行以下步骤:");
  console.log("1. 解压: tar -xzf my-tech-blog-clean.tar.gz");
